...
from . import algorithm-gpu-utils as utils
from . import algorithm-gpu-config as cfg
from . import algorithm-gpu-cpu-fallback as cpu
from . import algorithm-gpu-kernels as kernels
...
class PaletteMappingAlgorithmGPU:
    ...
    def _map_pixels_to_palette_gpu(self, image_array: np.ndarray, palette: List[List[int]], config: Dict[str, Any]) -> np.ndarray:
        ...
        self._cached_fields['palette_hsv'].from_numpy(
            cpu.color.rgb2hsv(palette_np / 255.0)
        )
