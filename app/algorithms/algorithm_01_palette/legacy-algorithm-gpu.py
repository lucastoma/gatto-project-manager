...
class PaletteMappingAlgorithmGPU:
        ...
        with self.profiler.profile_operation("map_pixels_to_palette_GPU", algorithm_id=self.algorithm_id):
            ...
            if use_fusion:
                ...
                self._find_closest_color_kernel(
                    fields['pixels_hsv'], fields['palette_hsv'],
                    fields['closest_indices'], fields['distance_modifier'],
                    config.get("hue_weight", 3.0)
                )
    ...
    @ti.kernel
    def _find_closest_color_kernel(self, pixels_hsv: ti.template(), palette_hsv: ti.template(),
                                 closest_indices: ti.template(), dist_modifier: ti.template(),
                                 hue_weight: ti.f32):
        """Kernel to find closest palette color for each pixel in HSV space.
        
        Args:
            pixels_hsv: Input pixels in HSV color space (shape: [num_pixels, 3])
            palette_hsv: Palette colors in HSV color space (shape: [num_colors, 3])
            closest_indices: Output array for closest palette indices (shape: [num_pixels])
            dist_modifier: Distance modifiers for each palette color (shape: [num_colors])
            hue_weight: Weight for hue component in distance calculation
        """
        for i in range(pixels_hsv.shape[0]):
            pixel_hsv = pixels_hsv[i]
            min_dist = 1e30
            best_j = 0
            valid_distance_found = False
