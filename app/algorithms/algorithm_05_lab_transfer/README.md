...
| **Hybrid** | Configurable pipeline of sequential transfer steps. | ✅ **Complete** | ✅ **Complete** with ping-pong buffer optimization | None. |ansfer modes. Supports intermediate step results. | ✅ **Implemented** (`process_image_hybrid`) | ✅ **Implemented** (`hybrid_transfer`). GPU path uses ping-pong buffers. Simpler steps (e.g., selective) operate efficiently on GPU. Complex steps like `adaptive` and `basic`/`linear_blend` (due to on-the-fly stats calculation) currently involve CPU roundtrips within the GPU pipeline. | **Medium**: Further optimize complex steps in GPU pipeline (e.g., full GPU-based stats for 'basic'/'linear_blend', refactor 'adaptive' for buffer-only ops) to eliminate remaining CPU-GPU transfers. |
